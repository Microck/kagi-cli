...
flowchart TD
    ...
    SearchCmd --> PreferAPI[Prefer API fallback to session]
    
    CheckReqs -->|Session Required| SessionReq[Use KAGI_SESSION_TOKEN]
    CheckReqs -->|API Required| APIReq[Use KAGI_API_TOKEN]
    CheckReqs -->|Both Accepted| BothAccepted[Try API first fallback to session]
    
    PreferAPI --> BothAccepted
```
...
4. **Scroll to "Session Link"** section
5. **Copy the full URL** that looks like:
   ```
...
3. **Select "API Settings"** or "API"
4. **Generate a new token** if you don't have one
...
```mermaid
flowchart TD
    Start([User runs: kagi search "query"]) --> CheckLens{CLI checks for lens flag}
    
    CheckLens -->|Lens specified| LensReq[Requires session_token]
    CheckLens -->|No lens| TryAPI[Try API token first]
    
    TryAPI --> APIToken{With API token}
    APIToken -->|Try Search API| SearchAPI{Search API}
    SearchAPI -->|Success| ReturnResults[Return results]
    SearchAPI -->|Auth error| Fallback[Fallback to session]
    APIToken -->|No API token| SessionPath[Use session_token path]
    
    SessionPath --> TryWeb{Session token path}
    TryWeb -->|Try web product| WebProd{Web product}
    WebProd -->|Success| ReturnResults
    WebProd -->|Error| ReportFail[Report failure]
    TryWeb -->|No session_token| Error[Error]
...
The base search command implements intelligent fallback:

**Scenario 1: API token works**
```bash
...
**Scenario 2: API token rejected, fallback to session**
```bash
export KAGI_API_TOKEN='valid_token'
...
3. Set token if missing: `kagi auth set --session-token '...'`
...
2. Set session token: `kagi auth set --session-token '...'`
...
3. Set API token: `kagi auth set --api-token '...'`
...
2. **Clear precedence:** Environment variables override config files
3. **Smart fallback:** Base search tries API first, falls back to session
4. **Security first:** Never commit tokens, rotate regularly, use secret managers
5. **Easy verification:** Use `kagi auth status` and `kagi auth check`
