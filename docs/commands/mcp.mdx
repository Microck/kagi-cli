...
Run a stdio MCP server that exposes Kagi tools for agents. By default the
server speaks MCP `2026-07-28`, the stateless protocol with per-request
metadata. Pass `--protocol stable` to serve the stable MCP specification with
the classic `initialize` handshake for clients that do not support the draft.

## Synopsis

```bash
kagi mcp [--protocol draft|stable] [--default-output json|toon|pretty|compact|markdown|csv] [--enable-mutating-tools]
kagi mcp install [--target CLIENT]... [--all] [--protocol draft|stable] [--dry-run]
kagi mcp setup [--target CLIENT]... [--all] [--protocol draft|stable] [--dry-run]
kagi mcp auth [--target CLIENT]... [--all] [--protocol draft|stable] [--dry-run]
```
...
Pass `--protocol stable` to write `--protocol stable` into the client launcher arguments, so the installed entry speaks the stable MCP specification with the `initialize` handshake. Without the flag, setup records the plain `mcp` command and the server keeps the default draft behavior:

```bash
kagi mcp install --target opencode --protocol stable
```
...
In the default draft mode, the server does not use the legacy `initialize`
handshake or protocol session. Every request, including `server/discover`,
sends the protocol version, client identity, and client capabilities in
`params._meta`. Clients can call `server/discover` first with a preferred
version and retry with a supported version if the server returns an
...
With `--protocol stable`, the server speaks the stable MCP specification
instead. It answers `initialize` by echoing the requested version when it is
one of `2025-11-25`, `2025-06-18`, `2025-03-26`, `2024-11-05`, or
`2024-10-07`, and otherwise falls back to `2025-11-25`. It also serves `ping`,
`tools/list`, and `tools/call`. Draft-only methods such as `server/discover`
return a `-32601` method-not-found error in stable mode.

Draft discovery and tool-list responses include `resultType`, `ttlMs`, and
