...
Translate bootstrap currently requires:

- `python3`
- the Python package `curl_cffi`

The CLI shells out once to mint `translate_session`, then uses normal Rust HTTP requests for the rest of the flow.
...
If you see an error mentioning `python3` or `curl_cffi`, install the missing dependency and retry.
