...
| 8 | Extend Assistant beyond the shipped custom-assistant surface | `useful: medium-high` `hard: medium-high` `risk: high` `scope: m` | Custom assistant CRUD and invocation by saved profile already shipped in v0.4.1. The remaining work is higher-level Assistant flows such as file context, save/share helpers, or mode helpers, not basic profile management. |
