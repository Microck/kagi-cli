...
| 3 | Add lens management commands | `useful: high` `hard: high` `risk: medium-high` `scope: l` | The CLI can consume a numeric lens id, but it cannot list, inspect, enable, disable, create, or update lenses. The docs make clear that lenses are a first-class Kagi feature and a major part of personalization. Candidate surface: `kagi lens list|get|enable|disable|create|update|delete`. Help pages: `/kagi/features/lenses.html`, `/kagi/settings/lenses.html` |
...
| 5 | Add redirects, custom bangs, and search-shortcut management | `useful: high` `hard: medium-high` `risk: high` `scope: l` | These are core power-user primitives in Kagi and a natural fit for a CLI. Candidate surface: `kagi redirect ...`, `kagi bang custom ...`, `kagi shortcut ...`, plus maybe `search --snap` as a convenience wrapper. Help pages: `/kagi/features/redirects.html`, `/kagi/features/bangs.html`, `/kagi/features/search-shortcuts.html`, `/kagi/features/snaps.html`, `/kagi/settings/advanced.html`, `/kagi/settings/search.html` |
...
| 8 | Add custom assistant CRUD and invocation by name/bang | `useful: high` `hard: high` `risk: high` `scope: l` | Kagi Assistant custom assistants are clearly a major feature now. The CLI can pass model/lens/web-access options, but cannot manage named assistants. Candidate surface: `kagi assistant custom list|create|update|delete|run`. Help pages: `/kagi/ai/custom-assistants.html`, `/kagi/settings/assistant.html`, `/kagi/ai/assistant.html` |
...
| 10 | Add richer Assistant workflow controls: share, save, and research modes | `useful: medium-high` `hard: high` `risk: high` `scope: l` | The CLI already handles prompting and thread export, but the docs mention saving threads, sharing threads, switching modes, and Kagi Research / Kagi Quick behavior. Candidate surface: `kagi assistant thread save`, `share`, and mode helpers like `--mode quick|research`. Help pages: `/kagi/ai/assistant.html`, `/kagi/ai/kagi-research.html` |
...
3. lens management
4. redirects + shortcuts + custom bangs
5. Assistant file/document context
