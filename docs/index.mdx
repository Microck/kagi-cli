---
title: "Introduction"
description: "Complete guide to the *kagi* CLI - a powerful, agent-native command-line interface for Kagi search, automation, and AI-powered workflows."
---

# *kagi* CLI Documentation

Welcome to the comprehensive documentation for `kagi`, a Rust-based command-line interface designed specifically for Kagi subscribers who demand powerful search capabilities, seamless automation, and agent-native workflows. This documentation provides everything you need to understand, install, configure, and master the *kagi* CLI.

## What is *kagi*?

`kagi` is a sophisticated command-line interface that bridges the gap between Kagi's powerful web product and your terminal. Built with Rust for performance and reliability, it provides structured JSON output by default while offering human-friendly formatting when you need it. The CLI wraps a mix of documented Kagi APIs, subscriber-only web-product flows, and public product endpoints behind a single, intuitive command surface.

### Core Philosophy

The *kagi* CLI was designed with three fundamental principles in mind:

1. **Automation-First**: Every command outputs structured JSON by default, making the CLI predictable and scriptable for automation, CI/CD pipelines, and agent workflows.
...
The CLI is specifically designed for agentic workflows:

- **Deterministic Output**: Consistent JSON schemas mean your agents can reliably parse and act on search results.
...
- **Assistant Integration**: Prompt Kagi's AI assistant programmatically and continue conversations across sessions.
...
- **Assistant**: Full conversation support with thread continuation
...
2. **Pretty Mode**: Human-friendly formatting with `--pretty` flag
...
kagi search --pretty "rust programming"
...
*Last updated: March 2026 | kagi CLI v0.1.6*
