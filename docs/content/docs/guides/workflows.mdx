...
# Check auth and adapt behavior; lens search needs a session token
if [ -n "$KAGI_SESSION_TOKEN" ] && kagi auth check 2>/dev/null && kagi auth status 2>/dev/null | grep -q 'selected: session-token'; then
