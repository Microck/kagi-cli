...
**Note:** Kagi's documentation notes that values other than `true` for `--web-search` are currently unsupported in practice. Passing `--web-search false` is rejected client-side with an explanatory error.
