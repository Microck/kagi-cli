...
- **Version Control Friendly**: Configuration lives in a simple TOML file. `kagi auth` writes credentials to `~/.config/kagi-cli/config.toml`, so keep that file private: commit only a redacted template and supply tokens via the wizard or environment variables.
...
- **No Interactive Prompts (mostly)**: Non-interactive subcommands are fully scriptable with flags and environment variables; the only exception is bare `kagi auth`, which launches an interactive setup wizard.
...
- **Account Settings**: Manage saved assistants, search lenses, custom bangs, and redirect rules without leaving the terminal.
- **Quick Answers**: Get Kagi Quick Answer responses with references and follow-up questions.
...
- **Translate Integration**: Run Kagi Translate text mode from the terminal with one JSON response envelope.
...
- **Snap-Prefixed Search**: Reuse your Kagi snap shortcuts from the CLI with `--snap`
- **API Key Path**: Uses the current documented Search API when you have API access
...
Three credential types serve different purposes:
...
- **KAGI_API_KEY**: Enables current `/api/v1` public API commands including Search and Extract. Requires API access and credit.

- **KAGI_API_TOKEN**: Enables legacy `/api/v0` public API commands including FastGPT, public Summarizer, and enrichment APIs. Requires API credit.
...
- **Assistant Profiles**: Reuse saved assistants with `--assistant` and manage them with `assistant custom`
- **Ask Page**: Page-focused Assistant questions with structured output
- **Settings Management**: Create and maintain lenses, custom bangs, and redirect rules
...
### Example: Quick Answers With References

The `kagi quick` command is a good fit when you want one synthesized answer instead of a full result list:
...
2. Local configuration file, resolved from `$KAGI_CONFIG` (an explicit full path), then `$XDG_CONFIG_HOME/kagi-cli/config.toml`, then `~/.config/kagi-cli/config.toml`
...
*Last updated: September 2026 | kagi CLI v0.19.0*
