...
| `search` with filters | `KAGI_SESSION_TOKEN` or `KAGI_API_KEY` | Session fallback only when API-first mode is enabled | Region, time-range, order, verbatim, and personalization filters are session-only; V1-compatible filters (region, from/to date, limit) also work with an API key |
