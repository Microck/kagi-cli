...
| `search` with filters | `KAGI_SESSION_TOKEN` or `KAGI_API_KEY` | Session fallback only when API-first mode is enabled | V1-compatible filters (`--region`, `--from-date`, `--to-date`) also work with an API key; `--time`, `--order`, `--verbatim`, and personalization filters remain session-only |
...
| `mail boxes/search/read` | Mail OAuth token | None | `kagi mail login` or `KAGI_MAIL_ACCESS_TOKEN`; separate from Search auth |
| `mail login/status/logout` | Private mail configuration | None | Device login, local status, and local token removal |
...
- **Purpose:** Validate the selected primary credential works
- **Network:** Yes; endpoint depends on credential: API key and session token run a test search, legacy API token validates against the FastGPT endpoint
...
```bash
kagi auth
# choose "Session Link" and paste your link; the token never lands in
...
```bash
kagi auth
# choose "Legacy API Token" and paste the value interactively
```
...
```bash
kagi auth
# run the wizard once per credential, choosing Session Link, API Key,
# then Legacy API Token; each paste is saved immediately
```

**All search, AI, and feed commands work. Mail requires separate OAuth login:**
...
### Adding a legacy API token to an existing Session setup

```bash
kagi auth
# already have a session token saved: choose "Legacy API Token" and paste it
...
```bash
kagi auth
# choose "Session Link" and paste your session link
