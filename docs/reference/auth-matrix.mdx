...
| `translate` | `KAGI_SESSION_TOKEN` | None | Text mode only; runtime bootstrap needs `python3 + curl_cffi` |
...
```mermaid
flowchart TD
    Start([User Query]) --> CheckAPI{KAGI_API_TOKEN configured?}
    
    CheckAPI -->|Yes| TrySearchAPI[Try Search API]
    TrySearchAPI -->|Success| ReturnResults1[Return results ✓]
    TrySearchAPI -->|Auth error| CheckSession1{KAGI_SESSION_TOKEN configured?}
    CheckSession1 -->|Yes| TryWeb1[Try web product]
    TryWeb1 -->|Success| ReturnResults2[Success ✓]
    CheckSession1 -->|No| Error1[Error: auth failed]
    
    CheckAPI -->|No| CheckSession2{KAGI_SESSION_TOKEN configured?}
    CheckSession2 -->|Yes| TryWeb2[Try web product]
    TryWeb2 -->|Success| ReturnResults3[Success ✓]
    CheckSession2 -->|No| Error2[Error: missing credentials]
```
